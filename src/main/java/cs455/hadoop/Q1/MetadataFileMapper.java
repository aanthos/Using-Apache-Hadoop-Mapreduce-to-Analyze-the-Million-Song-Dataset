...
public class MetadataFileMapper extends Mapper<LongWritable, Text, Text, Text> {
	...
	protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
		...
		if(title.isEmpty()) {
			title = song_id + " - No song title";
		}
	
		// emit artist_id, count pairs to Reducer
		// TODO Add "Metadata" string to tell Reducer what file to process?
		if(!song_id.equals("song_id") && !artist_id.equals("artist_id") && !title.equals("title")) {
			context.write(new Text(artist_id), 
				new Text("Metadata" + "," + artist_name + "," + song_id + "," + title + "," + "1"));
		}
	}
}
